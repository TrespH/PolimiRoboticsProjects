...
public:
    ...
    void tryMergeAndPublish() {
        ...
        if (std::abs(dt.toSec()) > 0.05) return; // Allow up to 50ms drift
...
private:
    ...
    const double MERGED_ANGLE_MIN = ANGLE_LEFT_CUT;
    const double MERGED_ANGLE_MAX = ANGLE_RIGHT_CUT + (ANGLE_RIGHT_CUT - ANGLE_LEFT_CUT);
	const int NUM_BINS = (int)((MERGED_ANGLE_MAX - MERGED_ANGLE_MIN) / ANGLE_INCREMENT);
